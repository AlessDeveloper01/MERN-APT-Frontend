...
const NuevoPassword = () => {
    ...
    useEffect(() =>  {
        const comprobarToken = async() => {
            try {
                ...
                setAlerta({
                    msg: 'Coloca el nuevo password que deseas',
                    error: false
                });
                setTokenValido(true);
            } catch (error) {
  ...
  return (
    <>
      <div>
        <h1 className="text-indigo-600 font-black text-6xl">Restablece Tu Password y no Pierdas a Tus {""} <span className="text-black">Pacientes</span></h1>
      </div>
      ...
      <div className='mt-20 md:mt-5 shadow-lg px-5 py-10 rounded-xl bg-white'>
        ...
        {tokenValido && (
            ...
            to="/">Se cambio tu password, ahora - <span className='text-indigo-500'>Inicia Sesion</span>
            </Link>
            )}
        </>
        )}
